...
const showlistSection = document.querySelector('.showlist')

if (showlistSection) {
	const showlistCard = showlistSection.querySelector('.showlist-card')
	const toggleButton = showlistSection.querySelector('.toggle-button')
	const toggleButton2 = showlistSection.querySelector('.toggle-button2')
	const playlistItems = Array.from(showlistSection.querySelectorAll('.showlistBox'))
	const playlistSeparators = Array.from(showlistSection.querySelectorAll('.separator'))

	const listTimers = []
	const revealDelay = 45
	const collapseDelay = 360
	const openMarkup =
	...
	let bottomButtonTimer = null

	const clearListTimers = () => {
		while (listTimers.length > 0) {
			window.clearTimeout(listTimers.pop())
		}

		if (bottomButtonTimer) {
			window.clearTimeout(bottomButtonTimer)
			bottomButtonTimer = null
		}
	}
	...
	const getExpandedCardHeight = () => {
		...
		const reservedHeight = isCompact ? 240 : 300

		return `${Math.max(320, viewportHeight - reservedHeight)}px`
	}

	const updateExpandedCardHeight = () => {
		if (!showlistSection.classList.contains('expanded')) {
			return
		}

		showlistCard.style.maxHeight = getExpandedCardHeight()
	}

	const setButtonsExpandedState = isExpanded => {
		toggleButton.innerHTML = isExpanded ? closeMarkup : openMarkup
		toggleButton2.innerHTML = closeMarkup
	}

	const resetPlaylistItems = () => {
		playlistItems.forEach(item => item.classList.remove('is-visible'))
		playlistSeparators.forEach(separator => separator.classList.remove('is-visible'))
	}

	const hideBottomButton = () => {
		toggleButton2.classList.remove('is-visible')
		bottomButtonTimer = window.setTimeout(() => {
			if (!toggleButton2.classList.contains('is-visible')) {
	...
	const showBottomButton = () => {
		...
		window.requestAnimationFrame(() => {
			toggleButton2.classList.add('is-visible')
		})
	}
	...
	const expandList = () => {
		clearListTimers()
		resetPlaylistItems()
		setButtonsExpandedState(true)
		hideBottomButton()

		showlistSection.classList.add('expanded')
		showlistCard.classList.add('is-open')
		showlistCard.style.maxHeight = getExpandedCardHeight()

		playlistItems.forEach((item, index) => {
			const revealTimer = window.setTimeout(() => {
				const previousSeparator = playlistSeparators[index - 1]

				if (previousSeparator) {
					previousSeparator.classList.add('is-visible')
				}
			...
			listTimers.push(revealTimer)
		})
	}

	const collapseList = shouldScrollToTop => {
		clearListTimers()
		setButtonsExpandedState(false)
		hideBottomButton()
		showlistCard.style.maxHeight = `${showlistCard.getBoundingClientRect().height}px`

		window.requestAnimationFrame(() => {
			showlistCard.classList.remove('is-open')
			showlistCard.style.maxHeight = '0px'
		})

		showlistSection.classList.remove('expanded')

		const resetTimer = window.setTimeout(() => {
			resetPlaylistItems()
		}, collapseDelay)

		listTimers.push(resetTimer)

		if (shouldScrollToTop) {
			const scrollTimer = window.setTimeout(() => {
				showlistSection.scrollIntoView({
					behavior: 'smooth',
					block: 'start',
				})
			}, 120)

			listTimers.push(scrollTimer)
		}
	}

	window.toggleList = function toggleList() {
		const isExpanded = showlistSection.classList.contains('expanded')
	...
	window.toggleListBottom = function toggleListBottom() {
		collapseList(true)
	}

	window.addEventListener('resize', updateExpandedCardHeight)
}
