...
;(function (ba) {
	...
	function U(b, c, d, e) {
		c &&
			B(c, function (c, g) {
				if (d || !t(b, g))
					e && 'object' === typeof c && c && !H(c) && !G(c) && !(c instanceof RegExp)
						? (b[g] || (b[g] = {}), U(b[g], c, d, e))
						: (b[g] = c)
			})
		return b
					...
					if ('string' === typeof c) {
						...
						return !t(r, n)
							? w(
									C(
										'notloaded',
										'Module name "' + n + '" has not been loaded yet for context: ' + b + (a ? '' : '. Use require([])')
									)
							  )
							: r[n]
					}
			...
			T(document.getElementsByTagName('script'), function (b) {
				...
				if ((I = b.getAttribute('data-main')))
					return (
						(s = I),
						q.baseUrl ||
							((E = s.split('/')), (s = E.pop()), (O = E.length ? E.join('/') + '/' : './'), (q.baseUrl = O)),
						(s = s.replace(Q, '')),
